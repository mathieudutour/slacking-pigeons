...
export async function subscribeToPremium (req: IncomingMessage, res: ServerResponse) {
  ...
  try {
    ...
    if (!err.type) {
      send(res, 500, upsell(body.teamId, `${err.message}. How did that happen!? Please ping me.`))
      return
    }
    switch (err.type) {
      ...
      case 'RateLimitError':
        ...
        send(res, 503, upsell(body.teamId, `Server is a bit overloaded, try again in a bit`))
        break
      ...
      case 'StripeAuthenticationError':
        ...
        send(res, 500, upsell(body.teamId, `How did that happen!? Please ping me.`))
        break
      default:
